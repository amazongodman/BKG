...
```{r}
train %>% group_by(user_id,utrip_id) %>% summarise(coun = n()) %>% group_by(user_id) %>% summarise(coun = n()) %>% arrange(desc(coun))
```
...
```{r}
train %>% count(hotel_country) %>% filter(n > 20)
...
```{r}
train %>% count(hotel_country) %>% arrange(n)
```
...
```{r}
train %>% head(50) %>% select(city_id) %>% unique()
```
...
```{r}
just_1 <- train %>% group_by(utrip_id) %>% summarise(coun=n()) %>% filter(coun == 1) %>% select(utrip_id) %>% unique()

train %>% filter(utrip_id %in% just_1$utrip_id) %>% count(hotel_country)
